...
<div class="main">
	...
	<OtherSkills list={cv.otherSkills} />
</div>
...
<style>
	.main {
		...
		height: 100%;
		padding: 16px 15px 25px 5px;
		font-size: 0.7rem;
	}
	
	.main > .body :global(> div + div) {
		margin-top: 12px;
	}

	.main > .body :global(section .content>ul) {
	...
	.main > .body :global(section .content > ul > li) {
		position: relative;
	}

	.main > .body :global(section .content > ul > li + li) {
		padding-top: 3px;
	}
